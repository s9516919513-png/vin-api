...
// -------------------- marketing API (ОСТАВЛЕНО КАК В РАБОЧЕЙ ВЕРСИИ) --------------------
async function fetchMarketing({ token, dealerId, startDate, endDate, siteSource = null }) {
  ...
  const body = {
    grouping: "periodDay",
    dealerIds: [dealerId], // <-- оставляем как у тебя работало
...
<body>
  <div class="container">
    <div>
      ...
      <div class="sub">Маркетинг — как в твоей рабочей версии (последние 30 дней, grouping: periodDay) + графики.</div>
    </div>
    ...
    <div class="card">
      ...
      <div class="muted" style="margin-top:10px">Если VIN неверный — покажем ошибку.</div>
    </div>
...
function safeTotal(marketing){
  return marketing?.total || {};
}
function safeChats(total){
  return total?.chats || {};
}
function safeSum(total){
  return (total?.sumWithBonusesExpenses ?? total?.sumExpenses);
}
...
function renderMarketing(marketing){
  ...
  return \`
    ...
    <div class="muted" style="margin-top:10px">
      Если по источнику пусто — данных нет за период или API не вернул разбивку.
    </div>
  \`;
}
...
async function checkVin(){
  ...
  try{
    ...
    out.innerHTML = \`
      <div class="card">
        <div class="titleRow">
          ...
          <div class="muted">dealerId: <b>\${esc(data?.marketing?.dealerId || data?.dealerId || '—')}</b></div>
        </div>
    ...
    if(data?.marketing?.ok){
      // графики рисуем только если маркетинг ok
...
// VIN -> нужные поля + маркетинг (ОСТАВЛЕНО ПО ЛОГИКЕ КАК У ТЕБЯ РАБОТАЛО)
app.get("/check-vin", async (req, res) => {
  ...
  try {
    ...
    // 2) маркетинг (последние 30 дней; ограничение ~31 день)
